# Напишите код, который анализирует число num и сообщает является ли оно простым или составным.
# Используйте правило для проверки: “Число является простым, если это число натуральное и
# делится нацело только на единицу и на себя”.
# Сделайте ограничение на ввод отрицательных чисел и чисел больше 100 тысяч.
# Если подается отрицательное число или число более ста тысяч,
# выведите на экран сообщение: "Число должно быть больше 1 и меньше 100000".
#
# Внимание! Число 1 — не является ни простым, ни составным числом, так как у него только один делитель — 1.

num = 29


def is_natural(number):
    """
    Проверка числа на натуральность (число должно быть целым и положительным)
    :param number:
    :return: True / False
    """
    if number > 0 and number == int(number):
        return True
    else:
        return False


def is_valid_input(number):
    """
    Проверка числа на валидность ввода (число должно быть не отрицательным и не больше 100 ьысяч)
    :param number:
    :return: True / False
    """
    if 1 < number <= 100000:
        return True
    else:
        return False


def is_simple(number):
    """
    Проверка является ли число простым.
    Простое число должно делиться нацело только на единицу и на себя.
    Достаточно найти один делитель кроме единицы и самого числа.
    :param number:
    :return: True / False
    """
    for i in range(2, number):
        if number % i == 0:
            return False
    return True


if is_valid_input(num):
    if is_natural(num):
        if is_simple(num):
            print('Простое')
        else:
            print('Не является простым')
    else:
        print('Не является натуральным')
else:
    print('Число должно быть больше 1 и меньше 100000')
