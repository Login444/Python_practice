...
with (
    open('names.txt', 'r', encoding='UTF-8') as file_names,
    open('numbers.txt', 'r', encoding='UTF-8') as file_numbers,
    open('new_file.txt', 'w', encoding='UTF-8') as new_file
):
    names = [row.strip() for row in file_names.readlines()]
    nums = [row.strip() for row in file_numbers.readlines()]
    numbers_1 = []
    for row in nums:
        first, second = row.split(' | ')
        numbers_1.append((int(first), float(second)))
    result = []
    max_len = len(max([names, numbers_1], key=len))
    for i in range(max_len):
        cur_number = numbers_1[i% len(numbers_1)][0] * numbers_1[i % len(numbers_1)][0]
        result.append((names[i % len(names)].upper(), round(cur_number)) if cur_number > 0
                      else (names[i % len(names)].lower(), abs(cur_number)))
    for row in result:
