...
# которая может быть использована для создания точно такого же объекта MyStrс теми же значениями value и author
import time
...
class MyStr(str):
    ...
    def __init__(self, value, author):
        self.value = value
        self.author = author
        self.time_creation = time.strftime("%Y-%m-%d %H:%M", time.localtime(time.time()))

    def __str__(self):
        return f'{self.value} (Автор: {self.author}, Время создания: {self.time_creation})'
